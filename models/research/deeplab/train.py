...
flags.DEFINE_float('adam_epsilon', 1e-8, 'Adam optimizer epsilon.')
...
def main(unused_argv):
  ...
  with tf.Graph().as_default() as graph:
    ...
    collection = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "logits")
    with tf.device(config.variables_device()):
      ...
      # grads_and_vars = [(g, v) for (g, v) in grads_and_vars if v not in collection]
